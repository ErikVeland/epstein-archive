...
WHERE COALESCE(junk_tier, 'clean') = 'clean'
  ...
  AND LOWER(full_name) !~* '\b(bluray|blu-ray|disc|rewritable|dumpster|hauls|columns|demolition|ditchin|postage|acoustics|personnel|persoanel)\b'
  AND LOWER(full_name) !~* '^(east|west|north|south)\s+(if|aft|aftstreet|street|road|avenue)\b'
  AND LOWER(full_name) !~* '\b(direction|provided)\s*$'
