...
content = content.replace(
  /const ents = Array.isArray\(r.entities\)/g,
  'const ents = Array.isArray((r as Record<string, unknown>).entities)',
);
content = content.replace(
  /r\.entities\.map/g,
  '((r as Record<string, unknown>).entities as unknown[]).map',
);
content = content.replace(/entities: ents/g, 'entities: ents as unknown as Person[]');
content = content.replace(
  /documents: r\.documents/g,
  'documents: (r as Record<string, unknown>).documents',
);
content = content.replace(
  /investigations: r\.investigations/g,
  'investigations: (r as Record<string, unknown>).investigations',
);
content = content.replace(
  /articles: r\.articles/g,
  'articles: (r as Record<string, unknown>).articles',
);
...
content = content.replace(
  /this\.fetchWithErrorHandling<unknown>\(`\S+\/readiness`\);/,
  'this.fetchWithErrorHandling<any>(`${API_BASE_URL}/readiness`, { useCache: false });',
);
