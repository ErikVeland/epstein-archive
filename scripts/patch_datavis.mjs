...
code = code.replace(
  ...
  'CustomTooltip = ({ active, payload, label }: { active?: boolean; payload?: Array<{ color?: string; fill?: string; name?: string; value: number }>; label?: string }) =>',
);
code = code.replace(
  /payload\.map\(\(entry\: any, index\: number\)/g,
...
code = code.replace(
  /\(entry: Record<string, unknown>, index\: number\)/g,
  '(entry: any, index: number)',
); // revert specific ones to not break inner loops, we'll patch them explicitly
