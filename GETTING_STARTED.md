...
DATABASE_URL=postgresql://epstein:epstein@localhost:5432/epstein_archive
```
...
```bash
# Recommended (Docker Postgres + migrate + minimal seed)
pnpm local:setup

# Or, if you already have Postgres running with DATABASE_URL configured:
pnpm db:migrate:pg
pnpm dev
