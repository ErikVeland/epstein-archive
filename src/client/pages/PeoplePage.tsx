  ...
  return (
    <ScopedErrorBoundary>
      <Profiler id="PeoplePage" onRender={onRenderCallback}>
        <div data-testid="people-page" className="stack-y gap-4 full-height">
          ...
          <Surface
            variant="quiet"
            className="relative z-40 p-4 stack-y md:stack-x v-center spread gap-3 no-shrink"
          >
            <div className="stack-x v-center gap-2">
              <Icon name="Users" size="sm" color="info" className="no-shrink" />
              <p className="text-[var(--text-primary)] text-sm">
                {total.toLocaleString()} subjects • Page {page}/{totalPagesLocal || 1}
              </p>
            </div>

            <div className="full-width md:w-auto grid grid-cols-1 sm:grid-cols-[1fr_1fr_auto] gap-2 md:stack-x v-center font-sans">
              {isAdmin && (
                <GlassButton
                  onClick={onAddSubject}
                  variant="secondary"
                  size="sm"
                  className="hidden md:stack-x v-center gap-2"
                >
              ...
              <EntityTypeFilter
                ...
                className="full-width md:w-auto"
              />
              ...
              <GlassButton
                ...
                variant="ghost"
                size="sm"
                className="h-11 w-11 no-shrink !px-0 !py-0"
          ...
          <div className="flex-fill min-h-[600px] full-width">
            {loading ? (
          ...
          {totalPagesLocal > 1 && (
            <div className="stack-x h-center gap-3 mt-4 no-shrink pb-4">
              <button
                onClick={() => setPage((p) => Math.max(1, p - 1))}
                disabled={page === 1}
                className="control px-4 text-[var(--text-primary)] disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[var(--glass-bg-highlight)] stack-x v-center gap-2"
              >
                <Icon name="ChevronLeft" size="sm" />
                <span>Previous</span>
              </button>

              <div className="chip px-4 h-11 stack-x v-center gap-2">
