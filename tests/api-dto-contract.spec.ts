...
test.describe('API DTO Contracts', () => {
  ...
  test('investigation case-folder evidence endpoints match shared DTO schemas', async ({
    ...
    if (investigations.length === 0) {
      expect(true, 'No investigations available in test dataset').toBeFalsy();
      return;
    }
  ...
  test('entity detail endpoint matches shared DTO schema', async ({ request }) => {
    ...
    if (entities.length === 0) {
      expect(true, 'No entities available in test dataset').toBeFalsy();
      return;
    }
  ...
  test('evidence detail endpoint matches shared DTO schema', async ({ request }) => {
    ...
    if (docs.length === 0) {
      expect(true, 'No documents available in test dataset').toBeFalsy();
      return;
    }
  ...
  test('entity with verified media returns non-empty media tab response', async ({ request }) => {
    ...
    if (!entityId) {
      expect(true, 'No entity with verifiedMedia > 0 found in test dataset').toBeFalsy();
      return;
    }
...
test.describe('Review Queue contract tests', () => {
  test('GET /review/mentions/queue returns valid mention queue shape', async ({ request }) => {
    ...
    if (res.status() === 401 || res.status() === 403) {
      expect(
        true,
        'Review queue requires admin auth — skipping in unauthenticated context',
      ).toBeFalsy();
      return;
    }
    if (!res.ok()) {
      expect(true, `Review mentions queue not available (${res.status()})`).toBeFalsy();
      return;
    }
  ...
  test('GET /review/claims/queue returns valid claims queue shape with entity names', async ({
    ...
    if (!res.ok()) {
      expect(true, `Review claims queue not available (${res.status()})`).toBeFalsy();
      return;
    }
    const payload = await res.json();
...
test.describe('Intelligence dashboard contract tests', () => {
  test('GET /intelligence/review returns valid review data shape', async ({ request }) => {
    ...
    if (!res.ok()) {
      expect(true, `Intelligence review endpoint not available (${res.status()})`).toBeFalsy();
      return;
    }
    const payload = await res.json();
  ...
  test('GET /intelligence/readiness returns valid readiness shape', async ({ request }) => {
    ...
    if (!res.ok()) {
      expect(true, `Intelligence readiness endpoint not available (${res.status()})`).toBeFalsy();
      return;
    }
    const payload = await res.json();
